...
export function renderBonusList(effects, title) {
  const rows = (effects || []).map((row) => {
    ...
    return `<li class="bonus-row" data-active="${row.active ? '1' : '0'}"><span class="bonus-src">${esc(src)}</span><span class="bonus-name">${esc(row.name || row.label || '')}</span><span class="bonus-rate">+${pct(rate)}</span><span class="bonus-live">${live}</span><span class="bonus-time">${esc(window)}</span></li>`
  })
